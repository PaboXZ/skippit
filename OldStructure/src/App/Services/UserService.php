...
class UserService {
    ...
    public function isEmailTaken(string $email){
        return (bool) $this->database->query("SELECT * FROM users WHERE email = :email", ['email' => $email])->recordExists();
    }

    public function isLoginTaken(string $login){
        return (bool) $this->database->query("SELECT * FROM users WHERE login = :login", ['login' => $login])->recordExists();
    }
